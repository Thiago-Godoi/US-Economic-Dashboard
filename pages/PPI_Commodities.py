...
st.set_page_config(page_title="PPI - Commodities", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Producer Price Index: All Commodities")
...
try:
    latest_date = get_latest_date("PPIACO")
    data = fetch_data("PPIACO", start_date=st.session_state.start_date, frequency=freq_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="PPI - Commodities Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download PPI - Commodities Data as Excel",
        data=excel_file,
        file_name="PPI_Commodities_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download PPI - Commodities Data as CSV",
        data=csv_file,
        file_name="PPI_Commodities_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Error fetching PPI - Commodities data: {e}")
