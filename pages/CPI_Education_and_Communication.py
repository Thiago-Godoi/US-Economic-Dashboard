...
# Page configuration
st.set_page_config(page_title="CPI - Education and Communication", layout="wide", initial_sidebar_state="collapsed")
...
# Title and Header
st.title("Consumer Price Index (CPI) - Education and Communication")
...
try:
    data = fetch_data("CPIEDUSL", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    if not data.empty:
        ...
        excel_file = BytesIO()
        data.to_excel(excel_file, index=True, sheet_name="Education and Communication CPI Data")
        excel_file.seek(0)
        st.download_button(
            label="💾 Download Education and Communication CPI Data as Excel",
            data=excel_file,
            file_name="Education_and_Communication_CPI_Data.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    else:
        st.warning("No data available for Education and Communication CPI.")
except Exception as e:
    st.error(f"Error fetching Education and Communication CPI data: {e}")
