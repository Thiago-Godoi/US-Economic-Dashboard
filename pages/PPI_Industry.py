...
st.set_page_config(page_title="PPI - Industry", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Producer Price Index by Industry: Total Manufacturing Industries")
...
try:
    latest_date = get_latest_date("PCUOMFGOMFG")
    data = fetch_data("PCUOMFGOMFG", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="PPI - Industry Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download PPI - Industry Data as Excel",
        data=excel_file,
        file_name="PPI_Industry_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download PPI - Industry Data as CSV",
        data=csv_file,
        file_name="PPI_Industry_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Error fetching PPI - Industry data: {e}")
