...
st.set_page_config(page_title="Layoffs", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Layoffs and Discharges: Total Nonfarm")
...
try:
    latest_date = get_latest_date("JTSLDL")
    data = fetch_data("JTSLDL", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Layoffs Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Layoffs Data as Excel",
        data=excel_file,
        file_name="Layoffs_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Layoffs Data as CSV",
        data=csv_file,
    ...
    st.error(f"Error fetching Layoffs data: {e}")
