...
st.set_page_config(page_title="CPI", layout="wide", initial_sidebar_state="collapsed")
...
try:
    latest_date = get_latest_date("CPIAUCSL")
    data = fetch_data("CPIAUCSL", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="CPI Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download CPI Data as Excel",
        data=excel_file,
        file_name="CPI_Data.xlsx",
...
st.markdown("---")
st.markdown("## CPI Opening: Subcategories")
...
cpi_opening = {
    ...
    "Recreation": "CPIRECSL",
    "Apparel": "CPIAPPNS",
    "Household Supplies": "CUUS0000SAH31",
    "Other Goods and Services": "CPIOGSSL"
}

columns_per_row = 2
subcategories = list(cpi_opening.items())

for i in range(0, len(subcategories), columns_per_row):
    cols = st.columns(min(columns_per_row, len(subcategories) - i))
    for col, (name, series_id) in zip(cols, subcategories[i:i + columns_per_row]):
        with col:
            
            try:
                data = fetch_data(series_id, frequency=freq_code)

                if not data.empty:
                    latest_row = data.iloc[-1]
                    value, date = latest_row["value"], latest_row.name.strftime("%Y-%m-%d")
                else:
                    value, date = "---", "---"
            except Exception as e:
            ...
            print(st.session_state.frequency)
            if st.button(f"{name} Details", key=name):
                nav_page(f"CPI_{name.replace(' ', '_')}")
