...
st.set_page_config(page_title="Retail Inventories", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Retailers: Inventories to Sales Ratio")
...
try:
    latest_date = get_latest_date("RETAILIRSA")
    data = fetch_data("RETAILIRSA", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Retail Inventories Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Retail Inventories Data as Excel",
        data=excel_file,
        file_name="Retail_Inventories_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Retail Inventories Data as CSV",
        data=csv_file,
        file_name="Retail_Inventories_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Error fetching Retail Inventories data: {e}")
