...
st.set_page_config(page_title="PCE", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Personal Consumption Expenditures (PCE)")
...
try:
    latest_date = get_latest_date("PCE")
    data = fetch_data("PCE", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="PCE Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download PCE Data as Excel",
        data=excel_file,
        file_name="PCE_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download PCE Data as CSV",
        data=csv_file,
        file_name="PCE_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Error fetching PCE data: {e}")
