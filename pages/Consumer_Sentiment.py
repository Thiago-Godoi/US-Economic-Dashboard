...
st.set_page_config(page_title="Consumer Sentiment", layout="wide", initial_sidebar_state="collapsed")
...
st.title("University of Michigan: Consumer Sentiment")
...
try:
    latest_date = get_latest_date("UMCSENT")
    data = fetch_data("UMCSENT", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Consumer Sentiment Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Consumer Sentiment Data as Excel",
        data=excel_file,
        file_name="Consumer_Sentiment_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Consumer Sentiment Data as CSV",
        data=csv_file,
        file_name="Consumer_Sentiment_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Error fetching Consumer Sentiment data: {e}")
