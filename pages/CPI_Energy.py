...
# Page configuration
st.set_page_config(page_title="CPI - Energy", layout="wide", initial_sidebar_state="collapsed")
...
# Title and Header
st.title("Consumer Price Index (CPI) - Energy")
...
try:
    data = fetch_data("CPIENGSL", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    if not data.empty:
        ...
        excel_file = BytesIO()
        data.to_excel(excel_file, index=True, sheet_name="Energy CPI Data")
        excel_file.seek(0)

        csv_file = data.to_csv(index=True).encode("utf-8")

        st.download_button(
            label="💾 Download Energy CPI Data as Excel",
            data=excel_file,
            file_name="Energy_CPI_Data.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
        st.download_button(
        label="📄 Download Energy CPI Data as CSV",
        data=csv_file,
        file_name="Energy_CPI_Data.csv",
        mime="text/csv"
    )
    else:
        st.warning("No data available for Energy CPI.")
except Exception as e:
    st.error(f"Error fetching Energy CPI data: {e}")
