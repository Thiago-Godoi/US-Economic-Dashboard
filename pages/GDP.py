...
st.set_page_config(page_title="GDP", layout="wide", initial_sidebar_state="collapsed")
...
try:
    latest_date = get_latest_date("GDP")
    data = fetch_data("GDP", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="GDP Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download GDP Data as Excel",
        data=excel_file,
        file_name="GDP_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download GDP Data as CSV",
        data=csv_file,
    ...
    st.markdown("GDP data is not available on a monthly basis. Please select a different frequency to view this window properly")
