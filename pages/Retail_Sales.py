...
st.set_page_config(page_title="Retail Sales", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Advance Retail Sales: Retail Trade and Food Services")
...
try:
    latest_date = get_latest_date("RSAFS")
    data = fetch_data("RSAFS", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Retail Sales Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Retail Sales Data as Excel",
        data=excel_file,
        file_name="Retail_Sales_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Retail Sales Data as CSV",
        data=csv_file,
    ...
    st.error(f"Error fetching Retail Sales data: {e}")
