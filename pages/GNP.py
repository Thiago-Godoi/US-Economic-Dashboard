...
st.set_page_config(page_title="GNP", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Gross National Product (GNP)")
...
try:
    latest_date = get_latest_date("GNP")
    data = fetch_data("GNP", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="GNP Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download GNP Data as Excel",
        data=excel_file,
        file_name="GNP_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download GNP Data as CSV",
        data=csv_file,
        file_name="GNP_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.markdown("GNP data is not available on a monthly basis. Please select a different frequency to view this window properly")
    print(f"Error fetching GNP data: {e}")
