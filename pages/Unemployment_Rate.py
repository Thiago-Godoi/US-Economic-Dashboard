...
st.set_page_config(page_title="Unemployment Rate", layout="wide", initial_sidebar_state="collapsed")
...
try:
    latest_date = get_latest_date("UNRATE")
    data = fetch_data("UNRATE", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Unemployment Rate Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Unemployment Rate Data as Excel",
        data=excel_file,
        file_name="Unemployment_Rate_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Unemployment Rate Data as CSV",
        data=csv_file,
        file_name="Unemployment_Rate_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.error(f"Error fetching Unemployment Rate data: {e}")
