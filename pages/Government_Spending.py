...
st.set_page_config(page_title="Government Spending", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Government Total Expenditures")
...
try:
    latest_date = get_latest_date("W068RCQ027SBEA")
    data = fetch_data("W068RCQ027SBEA", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Government Spending Data")
    excel_file.seek(0)

    csv_file = data.to_csv(index=True).encode("utf-8")

    st.markdown(f"First data from: {first_data_date}")
    st.markdown(f"Latest data from: {latest_date}")
    st.markdown('<div class="stDownloadButton">', unsafe_allow_html=True)
    st.download_button(
        label="💾 Download Government Spending Data as Excel",
        data=excel_file,
        file_name="Government_Spending_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Government Spending Data as CSV",
        data=csv_file,
        file_name="Government_Spending_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.markdown("Government Spending data is not available on a monthly basis. Please select a different frequency to view this window properly")
