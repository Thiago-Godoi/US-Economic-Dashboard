...
st.set_page_config(page_title="Construction", layout="wide", initial_sidebar_state="collapsed")
...
st.title("New Privately-Owned Housing Units Under Construction")
...
try:
    latest_date = get_latest_date("UNDCONTSA")
    data = fetch_data("UNDCONTSA", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Construction Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Construction Data as Excel",
        data=excel_file,
        file_name="Construction_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Construction Data as CSV",
        data=csv_file,
    ...
    st.error(f"Error fetching Construction data: {e}")
