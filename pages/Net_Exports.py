...
st.set_page_config(page_title="Net Exports", layout="wide", initial_sidebar_state="collapsed")
...
st.title("Net Exports of Goods and Services")
...
try:
    latest_date = get_latest_date("NETEXP")
    data = fetch_data("NETEXP", start_date=st.session_state.start_date, frequency=freq_code, units=units_code)
    ...
    excel_file = BytesIO()
    data.to_excel(excel_file, index=True, sheet_name="Net Exports Data")
    excel_file.seek(0)
    ...
    st.download_button(
        label="💾 Download Net Exports Data as Excel",
        data=excel_file,
        file_name="Net_Exports_Data.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
    st.download_button(
        label="📄 Download Net Exports Data as CSV",
        data=csv_file,
        file_name="Net_Exports_Data.csv",
        mime="text/csv"
    )
    st.markdown('</div>', unsafe_allow_html=True)
except Exception as e:
    st.markdown("Net Exports data is not available on a monthly basis. Please select a different frequency to view this window properly")
    print(f"Error fetching Net Exports data: {e}")
